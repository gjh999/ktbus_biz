...
public class EgovLoginServiceImpl extends EgovAbstractServiceImpl implements
	...
	public LoginVO nmcbLogin(LoginVO vo) throws Exception {
    	...
    	// 1. 입력한 비밀번호를 암호화한다.
    	String password = StringUtil.nvl(vo.getPassword());
    	if (!StringUtils.isEmpty(password)){
			...
			vo.setPassword(passwordEncoderForMigration.encode(password));
		}

    	// 2. 아이디와 암호화된 비밀번호가 DB와 일치하는지 확인한다.
    	LoginVO loginVO = loginDAO2.nmcbLogin(vo); // nmcb.org
