...
<!--	          <div class="content_wrap">-->
				<div id="contents" class="content" style="width:100%; padding-left:25px;">
	              <br>
	              <h1 class="tit_5">분할분담금 납부 안내장(지로) <button class='btn btn_excel' onclick="excelDown()" type="button" style="float:right;">엑셀다운로드</a></button> 
...
<script language="javascript1.2"  type="text/javaScript"> 
	...
	function excelDown(obj){
		if(sheet.getDataRows().length == 0){
			...
			sheet.exportData({
				merge:3,
				fileName:'분할분담금 납부 안내장(지로)'+'('+jname+')'+'.xlsx'
			});
		}
		function printDown(obj){
			// OUTPUT 변수 생성
			   var jocode = $('#JOCODE').val().padStart(6, '0'); // 조합코드 6자리
			   var fym = $('#FYM').val(); // 납입년월 6자리
			   var currentMonth = fym.substring(4, 6); // 월 2자리
			   
			   // 현재 날짜를 전송일자로 사용
			   var now = new Date();
			   var sendDate = now.getFullYear() + 
			                  (now.getMonth() + 1).toString().padStart(2, '0') + 
			                  now.getDate().toString().padStart(2, '0');
			   
			   // OUTPUT 조합: 총 22자리
			   var OUTPUT = jocode + fym + currentMonth + sendDate;
			   
			   // 출력 URL 생성
			   var printUrl = "https://biz.ktbus.com/MYSUIT/UView5/index.jsp?projectName=newmaa&formName=mm3600ji&biz_ktbus_parapra_snd_parm=" + OUTPUT;
	...
	function maxLengthCheck(object) {
		...
		$(document).ready(function() {
			...
			*/
			...
			var defaultYM = "202509";
			var currentYM = YY + "" + MM;
			//종료일						
			var YeDay = (currentYM > defaultYM) ? currentYM : defaultYM;
			//시작일
		    var ToDay = YY + "" + TMM;
			var TYM11 = "202501";
		...
		<!--
			...
			'init':{
			  ...
			  "Cfg": {
			    ...
			    //중앙(메인) 컬럼 설정
				"Cols": [
						{"Header": ["순번","순번"],"Name": "SEQN","Type": "Int","Width": 55, "Align":"Center"},
					  	{"Header": ["계약번호","계약번호"],"Name": "MBBUNH","Type": "Text","Format":"",MinWidth: 130, "Align":"Center"},
					  	{"Header": ["차량번호","차량번호"], "Name": "MKCHNO","Type": "Text","Format":"",Width: 145,RelWidth:1, "Align":"Center"},
						{"Header": ["분납회차","분납회차"],"Name": "MBBCNT","Type": "Text",Width:75 ,"Align":"Center"},
						{"Header": ["납입일자","납입일자"],"Name": "MBBNYMD","Type": "Text",Width: 100,"Align":"Center"},
						{"Header": ["분할분담금","분할분담금"],"Name": "MBBKUM","Type": "Int",Width: 90,RelWidth:1,},
					    {"Header": ["지로로 수납할 경우","지로번호"],"Name": "JIBUN","Type": "Text", Width : 80,RelWidth:1, "Align":"Center"},
					    {"Header": ["지로로 수납할 경우","통지번호"],"Name": "MBBSNO","Type": "Text", Width : 112,RelWidth:1, "Align":"Center"},
						{"Header": ["지로로 수납할 경우","전자납부번호"],"Name": "NAPBUN","Type": "Text", Width : 125,RelWidth:1,"Align":"Center"},
						{"Header": ["지로로 수납할 경우","전자납부마감일"],"Name": "JNAPYMD","Type": "Text", Width : 110,RelWidth:1, "Align":"Center"}
					  ]
			},
			//시트 이벤트
			'event':{
				/*onDataLoad:function(evtParam){
					sheet.addRow({"init":{"SEQN":"소계","MBBUNH":"소계","MKCHNO":"소계","MBBCNT":"소계","MBBNYMD":"소계", CanFocus:0}});
				}*/
			},
			//시트객체 생성
			'create':function () {
		...
		function doAction(str){			
			switch(str){
				case "search":
					...
					$('#FYM').val(tym);
					//데이터 조회
					sheet.doSearch({url: "<c:url value='/mss/maa/mm3600jList.do'/>", param: $("#listFrm").serialize()});
					break;
				case "exceldown":
				sheet.down2Excel({
					fileName: "TreeSheet",
